# SUSTech RAG Defense Full Script

这份文档是答辩时可以直接对着讲的“文档版 PPT”。它按正式答辩顺序组织：先讲项目目标，再讲系统流程、技术实现、实验结果、现场演示，最后准备常见追问。每一节都包含可以直接朗读的逐字稿，也包含必要的技术补充。
...
这里先不勾选本地 LLM，是因为我希望第一遍展示最稳定的证据抽取能力。对于学分、电话、邮箱、开放时间这类问题，抽取式回答更容易保持原文精确表达。
...
- Sources 包含学生工作部或心理咨询相关页面。
- 回答中应出现预约邮箱 `counseling@sustc.edu.cn`、电话 `88010576` 或地点 `荔园9栋一层105` 等证据信息。

逐字稿：

这个问题展示系统覆盖的不只是教学和图书馆，也包括学生服务类信息。对于校园知识库来说，这类问题很典型，因为用户往往问的是服务流程、联系方式和地点。
